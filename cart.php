...
$query="update tblorders set OrderNumber='$orderno',IsOrderPlaced='1', payment_mode='COD' where UserId='$userid' and IsOrderPlaced is null;";
...
<td colspan="3">
<input type="text" onkeypress="return /[a-z/A-Z]/i.test(event.key)" name="streename" placeholder="Street Name" class="form-control"  required="true">      
</td>
</tr>
...
<tr>
<td colspan="3"> 
<input type="text" onkeypress="return /[a-z/A-Z]/i.test(event.key)" name="city" placeholder="City" class="form-control" required="true">  
</td>
</tr>
<tr>
    <td colspan="3">
        <button type="submit" class="btn btn-danger" name="placeorder">COD</button>
        <?php
            $userid=base64_encode($userid);
            $amount=base64_encode($grandtotal);
        ?>
        <button type="submit" formaction="ordernow.php?userid=<?php echo $userid;?>&am=<?php echo $amount;?>"  class="btn btn-danger" onclick="add()">Pay Now</button>
    </td>
</tr>
   </form>
